...
def bar_chart(updated_df):
    ...
    if not updated_df[x_axis].dtype.name in ['object', 'category']:
    ...
    chart = px.bar(updated_df, x=x_axis, y=y_axis, title=f"Bar Chart: {x_axis} vs {', '.join(y_axis)}", template=theme)
